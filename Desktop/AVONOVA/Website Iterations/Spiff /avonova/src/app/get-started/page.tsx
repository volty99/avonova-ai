      ...
      <div className="w-full max-w-md relative">
                ...
                <React.Fragment key={step.id}>
                  <div className="flex flex-col items-center min-w-[160px]">
                    <span className="w-8 h-8 rounded-full flex items-center justify-center mb-2 relative">
                      {isCompleted ? (
                        <span className="w-8 h-8 rounded-full bg-[var(--accent)] flex items-center justify-center">
                        ...
                        <span className="w-8 h-8 rounded-full bg-[var(--accent)] flex items-center justify-center font-bold text-white text-base">{idx + 1}</span>
                      ) : (
                    ...
                    <span className={`font-semibold text-base ${isCurrent ? 'text-[var(--accent)]' : 'text-[var(--text-secondary)]'}`}>{t(step.title)}</span>
                  ...
                  {idx !== steps.length - 1 && (
                    <span key={`line-${idx}`} className="flex-shrink-0 px-6 flex items-center justify-center">
                      <svg width="60" height="4" viewBox="0 0 60 4" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <rect y="1" width="60" height="2" rx="1" fill={currentStep > idx + 1 ? 'var(--accent)' : 'var(--border)'} />
                      </svg>
                    </span>
                  )}
                </React.Fragment>
          ...
          <form onSubmit={nextStep} className="flex flex-col gap-4">
            {currentStep === 1 && (
              <>
                <div className="flex gap-2">
                  <input type="text" className="w-full px-4 py-3 rounded-lg border border-[var(--border)] bg-[var(--surface)] text-[var(--text-primary)] focus:border-[var(--accent)] outline-none transition-colors mb-4" placeholder={t('first_name')} value={formData.firstName} onChange={e => handleInputChange("firstName", e.target.value)} />
                  <input type="text" className="w-full px-4 py-3 rounded-lg border border-[var(--border)] bg-[var(--surface)] text-[var(--text-primary)] focus:border-[var(--accent)] outline-none transition-colors mb-4" placeholder={t('last_name')} value={formData.lastName} onChange={e => handleInputChange("lastName", e.target.value)} />
                </div>
                <input type="email" className="w-full px-4 py-3 rounded-lg border border-[var(--border)] bg-[var(--surface)] text-[var(--text-primary)] focus:border-[var(--accent)] outline-none transition-colors mb-4" placeholder={t('email')} value={formData.email} onChange={e => handleInputChange("email", e.target.value)} />
                <input type="password" className="w-full px-4 py-3 rounded-lg border border-[var(--border)] bg-[var(--surface)] text-[var(--text-primary)] focus:border-[var(--accent)] outline-none transition-colors mb-4" placeholder={t('password')} value={formData.password} onChange={e => handleInputChange("password", e.target.value)} />
                <input type="text" className="w-full px-4 py-3 rounded-lg border border-[var(--border)] bg-[var(--surface)] text-[var(--text-primary)] focus:border-[var(--accent)] outline-none transition-colors mb-4" placeholder={t('company')} value={formData.company} onChange={e => handleInputChange("company", e.target.value)} />
              </>
            )}
            {currentStep === 2 && (
              <>
                <select className="w-full px-4 py-3 rounded-lg border border-[var(--border)] bg-[var(--surface)] text-[var(--text-primary)] focus:border-[var(--accent)] outline-none transition-colors mb-4" value={formData.platform} onChange={e => handleInputChange("platform", e.target.value)}>
                  <option value="">Select Platform</option>
                  {platforms.map((p) => <option key={p} value={p}>{p}</option>)}
                </select>
                <select className="w-full px-4 py-3 rounded-lg border border-[var(--border)] bg-[var(--surface)] text-[var(--text-primary)] focus:border-[var(--accent)] outline-none transition-colors mb-4" value={formData.storeType} onChange={e => handleInputChange("storeType", e.target.value)}>
                  <option value="">Select Store Type</option>
                  {storeTypes.map((t) => <option key={t} value={t}>{t}</option>)}
                </select>
                <input type="text" className="w-full px-4 py-3 rounded-lg border border-[var(--border)] bg-[var(--surface)] text-[var(--text-primary)] focus:border-[var(--accent)] outline-none transition-colors mb-4" placeholder="Monthly Revenue" value={formData.monthlyRevenue} onChange={e => handleInputChange("monthlyRevenue", e.target.value)} />
              </>
            )}
            {currentStep === 3 && (
              <>
            ...
            {currentStep < 4 && (
              <>
                ...
                <div className="mt-6 text-center text-sm text-[var(--text-secondary)]">
                  {t('already_have_account')} <a href="/login" className="text-[var(--accent)] font-medium hover:underline dark:text-[var(--accent-dark)]">{t('login')}</a>
                </div>
              </>
            )}
            ...
            {currentStep === 4 && (
              <div className="flex flex-col items-center justify-center min-h-[180px]">
                <Link href="/pipeline-controller" className="bg-[var(--accent)] text-white px-6 py-3 rounded-full font-semibold hover:bg-[var(--accent-hover)] transition-colors text-lg block">{t('go_to_dashboard')}</Link>
              </div>
            )}
        ...
        {validationErrors.length > 0 && (
          <div className="absolute left-full top-1/2 transform -translate-y-1/2 ml-2 z-50">
            <div className="bg-blue-50 border border-[var(--accent)] rounded-lg p-4 min-w-[260px] max-w-xs shadow-lg">
              <div className="text-[var(--accent)] font-medium mb-1">{t('please_fix_errors')}</div>
              <ul className="text-[var(--accent)] text-sm space-y-1">
